...
BENCHMARK_CONFIGS: list[BenchmarkConfig] = [
    ...
    # ── Sample-size sweep (AICGaussNoPriors, moderate noise) ──────────
    BenchmarkConfig(
...
if __name__ == "__main__":
    ...
    out_path = "AICGaussNoPriors_benchmark_results.csv"
    results.to_csv(out_path, index=False)
    print(f"\nTrial results saved to {out_path}")

    # Save per-node interventional results
    if not node_results.empty:
        node_out_path = "AICGaussNoPriors_benchmark_interventional_nodes.csv"
